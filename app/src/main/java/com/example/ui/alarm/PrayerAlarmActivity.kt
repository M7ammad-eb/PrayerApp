...
class PrayerAlarmActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            val prefs = remember { com.example.data.preferences.PrayerPreferences(this@PrayerAlarmActivity) }
            val settings by prefs.settingsFlow.collectAsState(initial = null)
            
            MyApplicationTheme(
                themeMode = settings?.themeMode ?: com.example.data.models.AppThemeMode.SYSTEM,
                colorPreset = settings?.colorPreset ?: com.example.data.models.AppColorPreset.SYSTEM_DYNAMIC,
            ...
            if (!AthanAudioEngine.playbackState.value.isPlaying) {
                try {
                    ...
                    AthanAudioEngine.playSoundType(
                        ...
                        prayerType = prayerType,
                        audioStream = audioStream,
                        isArabic = (settings?.language ?: com.example.data.models.AppLanguage.SYSTEM).resolveIsArabic()
                    )
                }
            }
        }
